...
import { runGenesisAgent, runManagerAgent, runHistoryAgent, generateLoadingMessages } from './services/geminiService';
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-realm-dark text-realm-text font-sans p-4 custom-scrollbar selection:bg-realm-accent/40">
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 flex flex-col gap-8">
          <Card title={isGenesis ? 'Ready for Seeding' : `Epoch: ${bundle.world_state.time.day}`}>
            {isGenesis ? (
          ...
          <Card title="Chronicle" className="h-full flex flex-col">
             <div className="overflow-y-auto pr-3 space-y-4 flex-1 custom-scrollbar">
               {[...(bundle.world_state.event_log || [])].reverse().map((evt) => (
                 <div key={evt.id} className="bg-black/40 border-l-2 border-realm-accent p-4 rounded-xl group animate-in slide-in-from-right duration-500">
                   <div className="flex justify-between items-start mb-2"><span className="text-[8px] text-realm-muted uppercase tracking-widest">Day {evt.epoch}</span>{evt.decision_trace_id && <button onClick={() => { const t = bundle.world_state.decision_traces?.find(dt => dt.decision_trace_id === evt.decision_trace_id); if (t) setSelectedTrace(t); }} className="text-[9px] text-realm-accent hover:text-white underline">Trace</button>}</div>
